...
export async function createLead(input: LeadInput): Promise<WriteResult> {
  ...
  const { data: lead, error } = await supabase
    ...
    .insert({
      ...
      industry: input.answers?.industry ?? null,
      technician_band: input.answers?.technicians ?? null,
      business_model: input.answers?.businessModel ?? null,
      required_features: input.answers?.requiredFeatures ?? [],
      accounting: input.answers?.accounting ?? null,
      budget_band: input.answers?.budget ?? null,
